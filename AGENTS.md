...
本工作区是 **ArkAdmin（方舟）** 项目的家：一个基于 Laravel 重写 FastAdmin 的插件化后台框架（Laravel 12 + PHP 8.3 + PostgreSQL + Vue3/Element Plus 前后端分离 + NiuShop 式目录插件机制）。框架代码尚未创建，当前只有设计文档和两个只读参考仓库。

```
/home/gdmax/fastadmin/            # 工作区根（不是 git 仓库本身）
├── AGENTS.md                     # 本文件
